...
namespace Miner_WPF.Commons
{
    public class Configuration
    {
        ...
        internal static void Init(IMiner minerd, Action<string> action = null)
        {
            ...
            if (!File.Exists(file) || FileVersionInfo.GetVersionInfo(file).ProductVersion.CompareTo(appInfo.Version.Trim('\n')) != 0)
            {
            ...
            format = c => $"-a \"cryptonight\" -o \"stratum+tcp://{c.Url}\" -u \"{(string.IsNullOrEmpty(c.Id) ? c.User : $"{c.User}.{c.Id}")}\" -p \"{c.Pass}\" -t {c.Thread} --max-cpu-usage 75";
